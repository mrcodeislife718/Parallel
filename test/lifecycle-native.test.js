...
test('native Parallel kernel compiles independently of Node and runs tasks, timers, and descriptor readiness', async (t) => {
  ...
  assert.equal(kernel.ok, true, kernel.result?.stderr ?? kernel.result?.runtime?.stderr ?? kernel.result?.tcp?.stderr);
  assert.equal(kernel.manifest.abiVersion, 4);
